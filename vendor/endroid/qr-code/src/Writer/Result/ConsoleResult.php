...
final class ConsoleResult extends AbstractResult
{
    ...
    public function getString(): string
    {
        ...
        $marginLeft =
            $this->colorEscapeCode . self::TWO_BLOCKS[0] . self::TWO_BLOCKS[0];
        $marginRight =
            self::TWO_BLOCKS[0] . self::TWO_BLOCKS[0] . "\e[0m" . PHP_EOL;
        $marginVertical =
            $marginLeft . str_repeat(self::TWO_BLOCKS[0], $side) . $marginRight;
        ...
        for ($rowIndex = 0; $rowIndex < $side; $rowIndex += 2) {
            ...
            for ($columnIndex = 0; $columnIndex < $side; ++$columnIndex) {
                ...
                if ($rowIndex + 1 < $side) {
                    $combined |=
                        $matrix->getBlockValue($rowIndex + 1, $columnIndex) <<
                        1;
                }
