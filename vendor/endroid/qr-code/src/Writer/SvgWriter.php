...
final readonly class SvgWriter implements WriterInterface
{
    ...
    public function write(
        ...
        if (
            !isset($options[self::WRITER_OPTION_EXCLUDE_SVG_WIDTH_AND_HEIGHT])
        ) {
        ...
        $xml->addAttribute(
            "viewBox",
            "0 0 " . $matrix->getOuterSize() . " " . $matrix->getOuterSize()
        );
        ...
        $background->addAttribute(
            "fill-opacity",
            strval($qrCode->getBackgroundColor()->getOpacity())
        );
    ...
    private function writePath(
        ...
        for ($rowIndex = 0; $rowIndex < $matrix->getBlockCount(); ++$rowIndex) {
            ...
            for (
                $columnIndex = 0;
                $columnIndex < $matrix->getBlockCount();
                ++$columnIndex
            ) {
                if (1 === $matrix->getBlockValue($rowIndex, $columnIndex)) {
                    ...
                    if (
                        ...
                        $left =
                            ...
                            $matrix->getBlockSize() * $columnIndex;
                    }
                    // When we are at the
                    if (
                        $columnIndex === $matrix->getBlockCount() - 1 ||
                        0 ===
                            $matrix->getBlockValue($rowIndex, $columnIndex + 1)
                    ) {
                        $top =
                            $matrix->getMarginLeft() +
                            $matrix->getBlockSize() * $rowIndex;
                        $bottom =
                            $matrix->getMarginLeft() +
                            $matrix->getBlockSize() * ($rowIndex + 1);
                        $right =
                            $matrix->getMarginLeft() +
                            $matrix->getBlockSize() * ($columnIndex + 1);
                        $path .=
                            "M" .
                            $this->formatNumber($left) .
                            "," .
                            $this->formatNumber($top);
                        $path .=
                            "L" .
                            $this->formatNumber($right) .
                            "," .
                            $this->formatNumber($top);
                        $path .=
                            "L" .
        ...
        $pathDefinition->addAttribute(
            "fill-opacity",
            strval($qrCode->getForegroundColor()->getOpacity())
        );
        $pathDefinition->addAttribute("d", $path);
    }

    /** @param array<string, mixed> $options */
    private function writeBlockDefinitions(
        \SimpleXMLElement $xml,
        QrCodeInterface $qrCode,
        MatrixInterface $matrix,
        array $options
        ...
        $blockDefinition->addAttribute(
            "id",
            strval($options[self::WRITER_OPTION_BLOCK_ID])
        );
        $blockDefinition->addAttribute(
            "width",
            $this->formatNumber($matrix->getBlockSize())
        );
        $blockDefinition->addAttribute(
            "height",
            $this->formatNumber($matrix->getBlockSize())
    ...
    private function addLogo(
        ...
        $y =
            intval($xmlAttributes->height) / 2 -
            $logoImageData->getHeight() / 2;

        $imageDefinition = $xml->addChild("image");
        $imageDefinition->addAttribute("x", strval($x));
        $imageDefinition->addAttribute("y", strval($y));
        $imageDefinition->addAttribute(
            "width",
            strval($logoImageData->getWidth())
        );
        $imageDefinition->addAttribute(
            "height",
            strval($logoImageData->getHeight())
        );
