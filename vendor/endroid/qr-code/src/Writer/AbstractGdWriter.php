...
abstract readonly class AbstractGdWriter implements
    WriterInterface,
    ValidatingWriterInterface
{
    ...
    public function write(
        ...
        $baseBlockSize =
            RoundBlockSizeMode::None === $qrCode->getRoundBlockSizeMode()
                ? 10
                : intval($matrix->getBlockSize());
        ...
        if ($label instanceof LabelInterface) {
            ...
            $targetHeight +=
                $labelImageData->getHeight() +
                $label->getMargin()->getTop() +
                $label->getMargin()->getBottom();
        }
    ...
    private function addLogo(LogoInterface $logo, GdResult $result): GdResult
    {
        ...
        if ($logoImageData->getPunchoutBackground()) {
            ...
            $xOffsetStart = intval(
                $matrix->getOuterSize() / 2 - $logoImageData->getWidth() / 2
            );
            $yOffsetStart = intval(
                $matrix->getOuterSize() / 2 - $logoImageData->getHeight() / 2
            );
            for (
                $xOffset = $xOffsetStart;
                $xOffset < $xOffsetStart + $logoImageData->getWidth();
                ++$xOffset
            ) {
                for (
                    $yOffset = $yOffsetStart;
                    $yOffset < $yOffsetStart + $logoImageData->getHeight();
                    ++$yOffset
                ) {
                    imagesetpixel(
                        $targetImage,
    ...
    private function addLabel(LabelInterface $label, GdResult $result): GdResult
    {
        ...
        $x = intval(
            imagesx($targetImage) / 2 - $labelImageData->getWidth() / 2
        );
        ...
        if (LabelAlignment::Left === $label->getAlignment()) {
            ...
            $x =
                imagesx($targetImage) -
                $labelImageData->getWidth() -
                $label->getMargin()->getRight();
        }
    ...
    public function validateResult(
        ...
        if (!class_exists(QrReader::class)) {
            throw ValidationException::createForMissingPackage(
        ...
        if ($reader->text() !== $expectedData) {
            throw ValidationException::createForInvalidData(
                $expectedData,
                strval($reader->text())
            );
        }
