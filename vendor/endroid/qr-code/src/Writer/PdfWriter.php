...
final readonly class PdfWriter implements WriterInterface
{
    ...
    public function write(
        ...
        if (!class_exists(\FPDF::class)) {
            throw new \Exception(
                "Unable to find FPDF: check your installation"
            );
        }
        ...
        if (isset($options[self::WRITER_OPTION_PDF])) {
            ...
            $fpdf = new \FPDF("P", $unit, [
                $matrix->getOuterSize(),
                $matrix->getOuterSize() + $labelSpace,
            ]);
            $fpdf->AddPage();
        }
                ...
                if (1 === $matrix->getBlockValue($rowIndex, $columnIndex)) {
                    $fpdf->Rect(
                        $x +
                            ...
                            $columnIndex * $matrix->getBlockSize(),
                        $y +
                            $matrix->getMarginLeft() +
                            $rowIndex * $matrix->getBlockSize(),
                        $matrix->getBlockSize(),
                        $matrix->getBlockSize(),
                        "F"
                    );
        ...
        if (isset($options[self::WRITER_OPTION_LINK])) {
            ...
            $fpdf->Link(
                $x,
                $y,
                $x + $matrix->getOuterSize(),
                $y + $matrix->getOuterSize(),
                $link
            );
        }
    ...
    private function addLogo(
        ...
        if (null === $logoHeight || null === $logoWidth) {
            ...
            if (!$imageSize) {
                throw new \Exception(
                    sprintf(
                        'Unable to read image size for logo "%s"',
                        $logoPath
                    )
                );
            }
