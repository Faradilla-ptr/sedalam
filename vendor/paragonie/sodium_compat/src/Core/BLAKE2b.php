...
abstract class ParagonIE_Sodium_Core_BLAKE2b extends ParagonIE_Sodium_Core_Util
{
     ...
     * @throws SodiumException
     */
    protected static function add64(
        SplFixedArray $x,
        SplFixedArray $y
    ): SplFixedArray {
        $l = ($x[1] + $y[1]) & 0xffffffff;
        return self::new64((int) ($x[0] + $y[0] + ($l < $x[1] ? 1 : 0)), $l);
    }
     ...
     * @return void
     */
    protected static function store64(
        SplFixedArray $x,
        int $i,
        SplFixedArray $u
    ): void {
     ...
     * @throws TypeError
     */
    protected static function compress(
        SplFixedArray $ctx,
        SplFixedArray $buf
    ): void {
     ...
     * @return SplFixedArray
     */
    public static function stringToSplFixedArray(
        string $str = ""
    ): SplFixedArray {
        $values = unpack("C*", $str);
    ...
    public static function stringToContext(string $string): SplFixedArray
    {
        ...
        for ($i = 0; $i < 8; ++$i) {
            $ctx[0][$i] = SplFixedArray::fromArray([
                ...
                self::load_4(self::substr($string, $i << 3, 4)),
            ]);
        }
