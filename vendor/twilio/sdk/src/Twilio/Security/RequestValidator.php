...
class RequestValidator
{
    ...
    public function validate(
        ...
        return $validBodyHash &&
            ($validSignatureWithPort || $validSignatureWithoutPort);
    }
    ...
    static function unparse_url(array $parsedUrl): string
    {
        ...
        $parts["scheme"] = isset($parsedUrl["scheme"])
            ? $parsedUrl["scheme"] . "://"
            : "";
        $parts["user"] = $parsedUrl["user"] ?? "";
        $parts["pass"] = isset($parsedUrl["pass"])
            ? ":" . $parsedUrl["pass"]
            : "";
        $parts["pass"] =
            $parts["user"] || $parts["pass"] ? $parts["pass"] . "@" : "";
        $parts["host"] = $parsedUrl["host"] ?? "";
        $parts["port"] = isset($parsedUrl["port"])
            ? ":" . $parsedUrl["port"]
            : "";
        $parts["path"] = $parsedUrl["path"] ?? "";
        $parts["query"] = isset($parsedUrl["query"])
            ? "?" . $parsedUrl["query"]
            : "";
        $parts["fragment"] = isset($parsedUrl["fragment"])
            ? "#" . $parsedUrl["fragment"]
            : "";
