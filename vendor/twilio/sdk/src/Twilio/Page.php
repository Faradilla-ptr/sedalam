...
abstract class Page implements \Iterator
{
    ...
    protected function processResponse(Response $response)
    {
        if (
            ...
            !$this->isPagingEol($response->getContent())
        ) {
            $message =
                "[HTTP " .
    ...
    {
        return \array_key_exists("meta", $this->payload) &&
            \array_key_exists($key, $this->payload["meta"]);
    }
    ...
    public function getPreviousPageUrl(): ?string
    {
        if ($this->hasMeta("previous_page_url")) {
            ...
            \array_key_exists("previous_page_uri", $this->payload) &&
            $this->payload["previous_page_uri"]
        ) {
            return $this->getVersion()
                ->getDomain()
                ->absoluteUrl($this->payload["previous_page_uri"]);
        }
        return null;
    }
    ...
    public function getNextPageUrl(): ?string
    {
        if ($this->hasMeta("next_page_url")) {
            ...
            \array_key_exists("next_page_uri", $this->payload) &&
            $this->payload["next_page_uri"]
        ) {
            return $this->getVersion()
                ->getDomain()
                ->absoluteUrl($this->payload["next_page_uri"]);
        }
        return null;
    }
    ...
    public function nextPage(): ?Page
    {
        ...
        $response = $this->getVersion()
            ->getDomain()
            ->getClient()
            ->request("GET", $this->getNextPageUrl());
    ...
    public function previousPage(): ?Page
    {
        ...
        $response = $this->getVersion()
            ->getDomain()
            ->getClient()
            ->request("GET", $this->getPreviousPageUrl());
        return new static($this->getVersion(), $response, $this->solution);
    }
