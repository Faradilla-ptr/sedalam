...
class BaseClient
{
    ...
    public function setCredentialProvider(
    ...
    public function request(
        ...
        $logLevel =
            getenv("DEBUG_HTTP_TRAFFIC") === "true"
                ? "debug"
                : $this->getLogLevel();
        ...
        if ($this->userAgentExtensions) {
            $headers["User-Agent"] .=
                " " . implode(" ", $this->userAgentExtensions);
        }
    ...
    public function buildUri(string $uri): string
    {
        ...
        $parsedUrl["host"] = \implode(
            ".",
            \array_filter([$product, $newEdge, $newRegion, $domain])
        );
