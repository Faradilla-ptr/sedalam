...
EOT
     ...
     */
    public static function isHostInNoProxy(
        string $host,
        array $noProxyArray
    ): bool {
        if (\strlen($host) === 0) {
    ...
    {
        return (float) \function_exists("hrtime")
            ? \hrtime(true) / 1e9
            : \microtime(true);
    }
     ...
     * @internal
     */
    public static function idnUriConvert(
        UriInterface $uri,
        int $options = 0
    ): UriInterface {
        if ($uri->getHost()) {
     ...
     * @return string|false
     */
    private static function idnToAsci(
        string $domain,
        int $options,
        ?array &$info = []
    ) {
        ...
        throw new \Error(
            "ext-idn or symfony/polyfill-intl-idn not loaded or too old"
        );
    }
}
