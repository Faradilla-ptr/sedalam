...
class StreamHandler
{
    ...
    public function __invoke(
        ...
        if ("1.0" !== $protocolVersion && "1.1" !== $protocolVersion) {
            throw new ConnectException(
                sprintf(
                    "HTTP/%s is not supported by the stream handler.",
                    $protocolVersion
                ),
                $request
            );
    ...
    private function createResponse(
        ...
        try {
            [$ver, $status, $reason, $headers] = HeaderProcessor::parseHeaders(
                $hdrs
            );
        } catch (\Exception $e) {
    ...
    private function drain(
        ...
        Psr7\Utils::copyToStream(
            $source,
            $sink,
            \strlen($contentLength) > 0 && (int) $contentLength > 0
                ? (int) $contentLength
                : -1
        );
    ...
    {
        $errors = [];
        \set_error_handler(static function ($_, $msg, $file, $line) use (
            &$errors
        ): bool {
            $errors[] = [
                "message" => $msg,
    ...
    private function createStream(RequestInterface $request, array $options)
    {
        ...
        if (
            isset($options["on_headers"]) &&
            !\is_callable($options["on_headers"])
        ) {
        ...
        $contextResource = $this->createResource(static function () use (
            $context,
            $params
        ) {
    ...
    private function resolveHost(
        ...
        if (
            ...
            !\filter_var($uri->getHost(), \FILTER_VALIDATE_IP)
        ) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_crypto_method(
        RequestInterface $request,
        array &$options,
        $value,
        array &$params
        ...
        if (
            ...
            (defined("STREAM_CRYPTO_METHOD_TLSv1_3_CLIENT") &&
                $value === \STREAM_CRYPTO_METHOD_TLSv1_3_CLIENT)
        ) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_progress(
        RequestInterface $request,
        array &$options,
        $value,
        array &$params
    ): void {
        self::addNotification($params, static function (
            $code,
            $a,
            $b,
            $c,
    ...
    private function add_debug(
        ...
        $ident =
            $request->getMethod() . " " . $request->getUri()->withFragment("");
        self::addNotification($params, static function (
            int $code,
            ...$passed
        ) use ($ident, $value, $map, $args): void {
    ...
    private static function addNotification(
        array &$params,
        callable $notify
    ): void {
