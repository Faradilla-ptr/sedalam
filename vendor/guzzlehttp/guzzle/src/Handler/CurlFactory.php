...
class CurlFactory implements CurlFactoryInterface
{
        ...
        if ("2" === $protocolVersion || "2.0" === $protocolVersion) {
            if (!self::supportsHttp2()) {
                throw new ConnectException(
                    "HTTP/2 is supported by the cURL handler, however libcurl is built without HTTP/2 support.",
                    $request
                );
            }
    ...
    private static function supportsTls12(): bool
    {
        ...
        if (null === $supportsTls12) {
            $supportsTls12 =
                \CURL_SSLVERSION_TLSv1_2 & \curl_version()["features"];
        }
    ...
    private static function invokeStats(EasyHandle $easy): void
    {
        ...
        $curlStats["appconnect_time"] = \curl_getinfo(
            $easy->handle,
    ...
    private static function createRejection(
        ...
        if ("" !== $sanitizedError) {
            $redactedUriString = \GuzzleHttp\Psr7\Utils::redactUserInfo(
                $uri
            )->__toString();
            if (
                $redactedUriString !== "" &&
                false === \strpos($sanitizedError, $redactedUriString)
            ) {
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
                ...
                if (\is_string($options["verify"])) {
                    ...
                    if (
                        ...
                        (\is_link($options["verify"]) === true &&
                            ($verifyLink = \readlink($options["verify"])) !==
                                false &&
                            \is_dir($verifyLink))
                    ) {
        ...
        if (
            !isset($options["curl"][\CURLOPT_ENCODING]) &&
        ...
        if (!isset($options["sink"])) {
            ...
            $options["sink"] = \GuzzleHttp\Psr7\Utils::tryFopen(
                "php://temp",
                "w+"
            );
        }
        ...
        $easy->sink = $sink;
        $conf[\CURLOPT_WRITEFUNCTION] = static function ($ch, $write) use (
            $sink
        ): int {
        ...
        if (isset($options["connect_timeout"])) {
            ...
            $conf[\CURLOPT_CONNECTTIMEOUT_MS] =
        ...
        if (isset($options["crypto_method"])) {
            ...
            if ("2" === $protocolVersion || "2.0" === $protocolVersion) {
                if (
                    ...
                    throw new \InvalidArgumentException(
                        "Invalid crypto_method request option: unknown version provided"
                    );
                }
            } elseif (
                \STREAM_CRYPTO_METHOD_TLSv1_0_CLIENT ===
        ...
        if (isset($options["ssl_key"])) {
            if (\is_array($options["ssl_key"])) {
                if (\count($options["ssl_key"]) === 2) {
                    [$sslKey, $conf[\CURLOPT_SSLKEYPASSWD]] = $options[
                        "ssl_key"
                    ];
                } else {
        ...
        if (isset($options["progress"])) {
            ...
            $conf[\CURLOPT_PROGRESSFUNCTION] = static function (
                $resource,
                int $downloadSize,
                int $downloaded,
                int $uploadSize,
    ...
    private function createHeaderFn(EasyHandle $easy): callable
    {
        ...
        return static function (
            $ch,
            $h
        ) use ($onHeaders, $easy, &$startingResponse) {
            $value = \trim($h);
            if ($value === "") {
