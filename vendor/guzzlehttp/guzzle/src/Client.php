    ...
    private function buildUri(UriInterface $uri, array $config): UriInterface
    {
        if (isset($config["base_uri"])) {
            $uri = Psr7\UriResolver::resolve(
                Psr7\Utils::uriFor($config["base_uri"]),
                $uri
            );
        }

        if (
            isset($config["idn_conversion"]) &&
            $config["idn_conversion"] !== false
        ) {
            $idnOptions =
                $config["idn_conversion"] === true
                    ? \IDNA_DEFAULT
                    : $config["idn_conversion"];
    ...
    private function applyOptions(
        ...
        if (isset($options["headers"])) {
            if (
                ...
                range(0, count($options["headers"]) - 1)
            ) {
                throw new InvalidArgumentException(
                    "The headers array must have header name as keys."
                );
            }
        ...
        if (isset($options["form_params"])) {
            ...
            $options["_conditional"]["Content-Type"] =
        ...
        if (
            ...
            $modify["set_headers"]["Accept-Encoding"] =
        ...
        if (!empty($options["auth"]) && \is_array($options["auth"])) {
            ...
            switch ($type) {
                case "basic":
                    ...
                    $modify["set_headers"]["Authorization"] =
