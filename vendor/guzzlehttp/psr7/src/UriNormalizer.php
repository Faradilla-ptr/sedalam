...
final class UriNormalizer
{
    ...
    public static function normalize(
        ...
        if (
            $flags & self::REMOVE_DEFAULT_HOST &&
            $uri->getScheme() === "file" &&
        ...
        if (
            $flags & self::REMOVE_DEFAULT_PORT &&
            $uri->getPort() !== null &&
            Uri::isDefaultPort($uri)
        ) {
            $uri = $uri->withPort(null);
        }

        if (
            $flags & self::REMOVE_DOT_SEGMENTS &&
            !Uri::isRelativePathReference($uri)
        ) {
            $uri = $uri->withPath(
                UriResolver::removeDotSegments($uri->getPath())
            );
        }
    ...
    public static function isEquivalent(
        ...
        return (string) self::normalize($uri1, $normalizations) ===
            (string) self::normalize($uri2, $normalizations);
    }

    private static function capitalizePercentEncoding(
        UriInterface $uri
    ): UriInterface {
        $regex = "/(?:%[A-Fa-f0-9]{2})++/";
    ...
    private static function decodeUnreservedCharacters(
        UriInterface $uri
    ): UriInterface {
