...
final class Utils
{
    ...
    public static function modifyRequest(
        ...
        if (!empty($changes["set_headers"])) {
            $headers = self::caselessRemove(
                array_keys($changes["set_headers"]),
                $headers
            );
    ...
    public static function tryGetContents($stream): string
    {
        ...
        set_error_handler(static function (int $errno, string $errstr) use (
            ...
            $ex = new \RuntimeException(
                sprintf("Unable to read stream contents: %s", $errstr)
            );
        ...
        try {
            ...
            $ex = new \RuntimeException(
                sprintf("Unable to read stream contents: %s", $e->getMessage()),
                0,
                $e
            );
        }
