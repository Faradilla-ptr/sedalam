...
class Uri implements UriInterface, \JsonSerializable
{
     ...
     */
    public static function withQueryValues(
        UriInterface $uri,
        array $keyValueArray
    ): UriInterface {
    ...
    private static function getFilteredQueryString(
        ...
        return array_filter(explode("&", $current), function ($part) use (
            $decodedKeys
        ) {
            return !in_array(
                rawurldecode(explode("=", $part)[0]),
                $decodedKeys,
                true
            );
        });
    ...
    private static function generateQueryString(
        string $key,
        ?string $value
    ): string {
    ...
    private function validateState(): void
    {
        ...
        if ($this->getAuthority() === "") {
            ...
            if (
                $this->scheme === "" &&
                false !== strpos(explode("/", $this->path, 2)[0], ":")
            ) {
                throw new MalformedUriException(
