...
final class ReedSolomonCodec
{
    ...
    public function __construct(
        ...
        if ($symbolSize < 0 || $symbolSize > 8) {
            throw new InvalidArgumentException(
                "Symbol size must be between 0 and 8"
            );
        }

        if ($firstRoot < 0 || $firstRoot >= 1 << $symbolSize) {
            throw new InvalidArgumentException(
                "First root must be between 0 and " . (1 << $symbolSize)
            );
        }

        if ($numRoots < 0 || $numRoots >= 1 << $symbolSize) {
            throw new InvalidArgumentException(
                "Num roots must be between 0 and " . (1 << $symbolSize)
            );
        }

        if ($padding < 0 || $padding >= (1 << $symbolSize) - 1 - $numRoots) {
            throw new InvalidArgumentException(
                "Padding must be between 0 and " .
                    ((1 << $symbolSize) - 1 - $numRoots)
            );
        }
        ...
        $this->alphaTo = SplFixedArray::fromArray(
            array_fill(0, $this->blockSize + 1, 0),
            false
        );
        $this->indexOf = SplFixedArray::fromArray(
            array_fill(0, $this->blockSize + 1, 0),
            false
        );
        ...
        $this->generatorPoly = SplFixedArray::fromArray(
            array_fill(0, $numRoots + 1, 0),
            false
        );
        ...
        for (
            ...
            for ($j = $i; $j > 0; $j--) {
                if ($this->generatorPoly[$j] !== 0) {
                    $this->generatorPoly[$j] =
                        $this->generatorPoly[$j - 1] ^
                        $this->alphaTo[
                            $this->modNn(
                                $this->indexOf[$this->generatorPoly[$j]] + $root
                            )
                        ];
                } else {
            ...
            $this->generatorPoly[$j] =
                $this->alphaTo[
                    $this->modNn(
                        $this->indexOf[$this->generatorPoly[0]] + $root
                    )
                ];
        }
    ...
    public function encode(SplFixedArray $data, SplFixedArray $parity): void
    {
        ...
        for ($i = 0; $i < $iterations; ++$i) {
            ...
            if ($feedback !== $this->blockSize) {
                ...
                $feedback = $this->modNn(
                    $this->blockSize -
                        $this->generatorPoly[$this->numRoots] +
                        $feedback
                );
                ...
                for ($j = 1; $j < $this->numRoots; ++$j) {
                    $parity[$j] =
                        ...
                        $this->alphaTo[
                            $this->modNn(
                                $feedback +
                                    $this->generatorPoly[$this->numRoots - $j]
                            )
                        ];
                }
            }
            ...
            if ($feedback !== $this->blockSize) {
                $parity[$this->numRoots - 1] =
                    $this->alphaTo[
                        $this->modNn($feedback + $this->generatorPoly[0])
                    ];
            } else {
    ...
    public function decode(
        SplFixedArray $data,
        ?SplFixedArray $erasures = null
    ): ?int {
        // This speeds up the initialization a bit.
        $numRootsPlusOne = SplFixedArray::fromArray(
            array_fill(0, $this->numRoots + 1, 0),
            false
        );
        $numRoots = SplFixedArray::fromArray(
            array_fill(0, $this->numRoots, 0),
            false
        );
        ...
        $syndromes = SplFixedArray::fromArray(
            array_fill(0, $this->numRoots, $data[0]),
            false
        );
        ...
        if ($numErasures > 0) {
            ...
            $lambda[1] =
                $this->alphaTo[
                    $this->modNn(
                        $this->primitive * ($this->blockSize - 1 - $erasures[0])
                    )
                ];

            for ($i = 1; $i < $numErasures; ++$i) {
                $u = $this->modNn(
                    $this->primitive * ($this->blockSize - 1 - $erasures[$i])
                );
        ...
        while (++$r <= $this->numRoots) {
            ...
            for ($i = 0; $i < $r; ++$i) {
                if (
                    $lambda[$i] !== 0 &&
                    $syndromes[$r - $i - 1] !== $this->blockSize
                ) {
                    $discrepancyR ^=
                        $this->alphaTo[
                            $this->modNn(
            ...
            for ($i = 0; $i < $this->numRoots; ++$i) {
                if ($b[$i] !== $this->blockSize) {
                    $t[$i + 1] =
                        $lambda[$i + 1] ^
                        $this->alphaTo[$this->modNn($discrepancyR + $b[$i])];
                } else {
                    $t[$i + 1] = $lambda[$i + 1];
                }
            }
        ...
        for ($i = 0; $i <= $degOmega; ++$i) {
            ...
            for ($j = $i; $j >= 0; --$j) {
                if (
                    ...
                    $tmp ^=
                        $this->alphaTo[
                            $this->modNn($syndromes[$i - $j] + $lambda[$j])
                        ];
                }
            }
        ...
        for ($j = $count - 1; $j >= 0; --$j) {
            ...
            $num2 =
                $this->alphaTo[
                    $this->modNn(
                        $root[$j] * ($this->firstRoot - 1) + $this->blockSize
                    )
                ];
            $den = 0;
            ...
            for (
                $i = min($degLambda, $this->numRoots - 1) & ~1;
                $i >= 0;
                $i -= 2
            ) {
                if ($lambda[$i + 1] !== $this->blockSize) {
                    $den ^=
                        $this->alphaTo[
                            $this->modNn($lambda[$i + 1] + $i * $root[$j])
                        ];
                }
            }
