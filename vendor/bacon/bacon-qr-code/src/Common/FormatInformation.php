...
class FormatInformation
{
     ...
     * @var int[]
     */
    private const BITS_SET_IN_HALF_BYTE = [
        0,
        1,
        1,
        2,
    ...
    {
        $this->ecLevel = ErrorCorrectionLevel::forBits(
            ($formatInfo >> 3) & 0x3
        );
    ...
    public static function numBitsDiffering(int $a, int $b): int
    {
        ...
        return self::BITS_SET_IN_HALF_BYTE[$a & 0xf] +
            self::BITS_SET_IN_HALF_BYTE[
                BitUtils::unsignedRightShift($a, 4) & 0xf
            ] +
            self::BITS_SET_IN_HALF_BYTE[
                BitUtils::unsignedRightShift($a, 8) & 0xf
            ] +
            self::BITS_SET_IN_HALF_BYTE[
                BitUtils::unsignedRightShift($a, 12) & 0xf
            ] +
            self::BITS_SET_IN_HALF_BYTE[
                BitUtils::unsignedRightShift($a, 16) & 0xf
            ] +
            self::BITS_SET_IN_HALF_BYTE[
                BitUtils::unsignedRightShift($a, 20) & 0xf
            ] +
            self::BITS_SET_IN_HALF_BYTE[
                BitUtils::unsignedRightShift($a, 24) & 0xf
            ] +
            self::BITS_SET_IN_HALF_BYTE[
                BitUtils::unsignedRightShift($a, 28) & 0xf
            ];
    }
     ...
     */
    public static function decodeFormatInformation(
        int $maskedFormatInfo1,
        int $maskedFormatInfo2
    ): ?self {
        $formatInfo = self::doDecodeFormatInformation(
            $maskedFormatInfo1,
            $maskedFormatInfo2
        );
    ...
    private static function doDecodeFormatInformation(
        ...
        foreach (self::FORMAT_INFO_DECODE_LOOKUP as $decodeInfo) {
            ...
            $bitsDifference = self::numBitsDiffering(
                $maskedFormatInfo1,
                $targetInfo
            );
    ...
    {
        return $this->ecLevel === $other->ecLevel &&
