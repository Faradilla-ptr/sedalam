...
final class Encoder
{
    ...
    public static function encode(
        ...
        if (
            $prefixEci &&
            Mode::BYTE() === $mode &&
            self::DEFAULT_BYTE_MODE_ENCODING !== $encoding
        ) {
        ...
        $provisionalBitsNeeded =
            ...
            $dataBits->getSize();
        $provisionalVersion = self::chooseVersion(
            $provisionalBitsNeeded,
            $ecLevel
        );
        ...
        $maskPattern = self::chooseMaskPattern(
            $finalBits,
            $ecLevel,
            $version,
            $matrix
     ...
     */
    private static function chooseMode(
        string $content,
        ?string $encoding = null
    ): Mode {
        if (null !== $encoding && 0 === strcasecmp($encoding, "SHIFT-JIS")) {
            return self::isOnlyDoubleByteKanji($content)
                ? Mode::KANJI()
                : Mode::BYTE();
        }
     ...
     */
    private static function chooseVersion(
        int $numInputBits,
        ErrorCorrectionLevel $ecLevel
    ): Version {
     ...
     */
    private static function terminateBits(
        int $numDataBytes,
        BitArray $bits
    ): void {
    ...
    private static function interleaveWithEcBytes(
        ...
        for ($i = 0; $i < $numRsBlocks; ++$i) {
            list(
                $numDataBytesInBlock,
                $numEcBytesInBlock,
            ) = self::getNumDataBytesAndNumEcBytesForBlockId(
                $numTotalBytes,
                $numDataBytes,
                $numRsBlocks,
                $i
     ...
     */
    private static function generateEcBytes(
     ...
     */
    private static function getCodec(
        int $numDataBytes,
        int $numEcBytesInBlock
    ): ReedSolomonCodec {
     ...
     */
    private static function appendLengthInfo(
        int $numLetters,
        Version $version,
        Mode $mode,
        BitArray $bits
     ...
     */
    private static function appendBytes(
        string $content,
        Mode $mode,
        BitArray $bits,
        string $encoding
     ...
     */
    private static function appendNumericBytes(
        string $content,
        BitArray $bits
    ): void {
     ...
     */
    private static function appendAlphanumericBytes(
        string $content,
        BitArray $bits
    ): void {
        $length = strlen($content);
     ...
     */
    private static function append8BitBytes(
        string $content,
        BitArray $bits,
        string $encoding
    ): void {
        ...
        if (false === $bytes) {
            throw new WriterException(
                "Could not encode content to " . $encoding
            );
        }
     ...
     */
    private static function appendKanjiBytes(
        string $content,
        BitArray $bits
    ): void {
        $bytes = @iconv("utf-8", "SHIFT-JIS", $content);
        ...
        if (false === $bytes) {
            throw new WriterException(
                "Content could not be converted to SHIFT-JIS"
            );
        }
     ...
     */
    private static function appendEci(
        CharacterSetEci $eci,
        BitArray $bits
    ): void {
        $mode = Mode::ECI();
