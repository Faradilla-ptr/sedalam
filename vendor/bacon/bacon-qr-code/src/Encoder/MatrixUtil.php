...
final class MatrixUtil
{
     ...
     */
    private static function embedTypeInfo(
        ErrorCorrectionLevel $level,
        int $maskPattern,
        ByteMatrix $matrix
    ): void {
     ...
     */
    private static function makeTypeInfoBits(
        ErrorCorrectionLevel $level,
        int $maskPattern,
        BitArray $bits
    ): void {
        ...
        if (15 !== $bits->getSize()) {
            throw new RuntimeException(
                "Bit array resulted in invalid size: " . $bits->getSize()
            );
        }
    }
     ...
     */
    private static function maybeEmbedVersionInfo(
        Version $version,
        ByteMatrix $matrix
    ): void {
     ...
     * @throws RuntimeException if bit array resulted in invalid size
     */
    private static function makeVersionInfoBits(
        Version $version,
        BitArray $bits
    ): void {
        $bits->appendBits($version->getVersionNumber(), 6);

        $bchCode = self::calculateBchCode(
            $version->getVersionNumber(),
            self::VERSION_INFO_POLY
        );
     ...
     */
    private static function embedBasicPatterns(
        Version $version,
        ByteMatrix $matrix
    ): void {
     ...
     */
    private static function embedPositionDetectionPatternsAndSeparators(
        ByteMatrix $matrix
    ): void {
        $pdpWidth = count(self::POSITION_DETECTION_PATTERN[0]);

        self::embedPositionDetectionPattern(0, 0, $matrix);
        self::embedPositionDetectionPattern(
            $matrix->getWidth() - $pdpWidth,
            0,
            $matrix
        );
        ...
        self::embedHorizontalSeparationPattern(
        ...
        self::embedVerticalSeparationPattern(
     ...
     */
    private static function embedPositionDetectionPattern(
        int $xStart,
        int $yStart,
        ByteMatrix $matrix
    ): void {
        for ($y = 0; $y < 7; ++$y) {
            for ($x = 0; $x < 7; ++$x) {
                $matrix->set(
                    $xStart + $x,
                    $yStart + $y,
                    self::POSITION_DETECTION_PATTERN[$y][$x]
                );
            }
        }
    }

    private static function removePositionDetectionPattern(
        int $xStart,
        int $yStart,
        ByteMatrix $matrix
    ): void {
     ...
     */
    private static function embedHorizontalSeparationPattern(
        int $xStart,
        int $yStart,
        ByteMatrix $matrix
    ): void {
     ...
     * @throws RuntimeException if a byte was already set
     */
    private static function embedVerticalSeparationPattern(
        int $xStart,
        int $yStart,
        ByteMatrix $matrix
    ): void {
     ...
     */
    private static function embedDarkDotAtLeftBottomCorner(
        ByteMatrix $matrix
    ): void {
     ...
     */
    private static function maybeEmbedPositionAdjustmentPatterns(
        Version $version,
        ByteMatrix $matrix
    ): void {
        ...
        $coordinates =
            self::POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE[$index];
        ...
        for ($i = 0; $i < $numCoordinates; ++$i) {
            for ($j = 0; $j < $numCoordinates; ++$j) {
                ...
                if (-1 === $matrix->get($x, $y)) {
                    self::embedPositionAdjustmentPattern(
                        $x - 2,
                        $y - 2,
                        $matrix
                    );
     ...
     */
    private static function embedPositionAdjustmentPattern(
        int $xStart,
        int $yStart,
        ByteMatrix $matrix
    ): void {
        for ($y = 0; $y < 5; $y++) {
            for ($x = 0; $x < 5; $x++) {
                $matrix->set(
                    $xStart + $x,
                    $yStart + $y,
                    self::POSITION_ADJUSTMENT_PATTERN[$y][$x]
                );
            }
        }
    }
     ...
     */
    private static function embedDataBits(
        BitArray $dataBits,
        int $maskPattern,
        ByteMatrix $matrix
    ): void {
