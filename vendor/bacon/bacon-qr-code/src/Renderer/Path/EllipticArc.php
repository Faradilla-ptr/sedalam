...
{
    private const ZERO_TOLERANCE = 1e-5;
    ...
    {
        if (
            sqrt(($fromX - $this->x) ** 2 + ($fromY - $this->y) ** 2) <
            self::ZERO_TOLERANCE
        ) {
            return [];
        }
        ...
        if (
            $this->xRadius < self::ZERO_TOLERANCE ||
            $this->yRadius < self::ZERO_TOLERANCE
        ) {
     ...
     * @return float[]
     */
    private function calculateCenterPointParameters(
        float $fromX,
        float $fromY,
        float $xAngle
    ): array {
        ...
        // F.6.5.3
        $cx =
            cos($xAngle) * $cxp - sin($xAngle) * $cyp + ($fromX + $this->x) / 2;
        $cy =
            sin($xAngle) * $cxp + cos($xAngle) * $cyp + ($fromY + $this->y) / 2;
    ...
    private static function point(
        ...
        return [
            $centerX +
            $radiusX * cos($xAngle) * cos($angle) -
            $radiusY * sin($xAngle) * sin($angle),
            $centerY +
            $radiusX * sin($xAngle) * cos($angle) +
            $radiusY * cos($xAngle) * sin($angle),
        ];
    }
     ...
     * @return float[]
     */
    private static function derivative(
        float $radiusX,
        float $radiusY,
        float $xAngle,
        float $angle
        ...
        return [
            -$radiusX * cos($xAngle) * sin($angle) -
            $radiusY * sin($xAngle) * cos($angle),
            -$radiusX * sin($xAngle) * sin($angle) +
            $radiusY * cos($xAngle) * cos($angle),
        ];
    }
}
