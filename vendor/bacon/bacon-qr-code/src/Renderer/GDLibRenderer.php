...
final class GDLibRenderer implements RendererInterface
{
    ...
    public function __construct(
        ...
        if (!extension_loaded("gd") || !function_exists("gd_info")) {
            throw new RuntimeException(
                "You need to install the GD extension to use this back end"
            );
        }
        ...
        if ($this->fill->hasGradientFill()) {
            throw new InvalidArgumentException(
                "GDLibRenderer does not support gradients"
            );
        }
    }
    ...
    public function render(QrCode $qrCode): string
    {
        ...
        if ($matrixSize !== $matrix->getHeight()) {
            throw new InvalidArgumentException(
                "Matrix must have the same width and height"
            );
        }
    ...
    private function draw(ByteMatrix $matrix): void
    {
        ...
        $this->drawEye(
            ...
            0,
            $pointInPx,
            $this->fill->getTopRightEyeFill()
        );
        $this->drawEye(
            0,
            $matrixSize - 7,
            $pointInPx,
            $this->fill->getBottomLeftEyeFill()
        );
    ...
    private function renderImage(): string
    {
        ...
        switch ($this->imageFormat) {
            ...
            default:
                ob_end_clean();
                throw new InvalidArgumentException(
                    "Supported image formats are jpeg, png and gif, got: " .
                        $this->imageFormat
                );
        }
    ...
    private function getColor(ColorInterface $color): int
    {
        ...
        if (!isset($this->colors[$colorKey])) {
            ...
            if ($colorId === false) {
                throw new RuntimeException(
                    "Failed to create color: #" . $colorKey
                );
            }
