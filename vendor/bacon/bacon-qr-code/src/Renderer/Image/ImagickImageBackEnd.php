...
final class ImagickImageBackEnd implements ImageBackEndInterface
{
    ...
    public function new(int $size, ColorInterface $backgroundColor): void
    {
        ...
        $this->image->newImage(
            $size,
            $size,
            $this->getColorPixel($backgroundColor)
        );
    ...
    public function scale(float $size): void
    {
        ...
        $this->matrices[$this->matrixIndex] = $this->matrices[
            $this->matrixIndex
    ...
    public function push(): void
    {
        ...
        $this->draw->push();
        $this->matrices[++$this->matrixIndex] =
            $this->matrices[$this->matrixIndex - 1];
    }
    ...
    private function createGradientFill(
        ...
        list($width, $height) = $this->matrices[$this->matrixIndex]->apply(
            $width,
            $height
        );

        $startColor = $this->getColorPixel(
            $gradient->getStartColor()
        )->getColorAsString();
        $endColor = $this->getColorPixel(
        ...
        switch ($gradient->getType()) {
            case GradientType::HORIZONTAL():
                $gradientImage->newPseudoImage(
                    (int) $height,
                    (int) $width,
                    sprintf("gradient:%s-%s", $startColor, $endColor)
                );
            ...
            case GradientType::RADIAL():
                $gradientImage->newPseudoImage(
                    (int) $width,
                    (int) $height,
                    sprintf("radial-gradient:%s-%s", $startColor, $endColor)
                );
                break;
        }
    ...
    private function getColorPixel(ColorInterface $color): ImagickPixel
    {
        ...
        if ($color instanceof Gray) {
            return new ImagickPixel(
                sprintf("graya(%d%%, %F)", $color->getGray(), $alpha / 100)
            );
        }
