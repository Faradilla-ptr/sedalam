...
final class SvgImageBackEnd implements ImageBackEndInterface
{
    ...
    private const TRANSLATE_FORMAT =
        "translate(%." . self::PRECISION . "F,%." . self::PRECISION . "F)";
    ...
    public function new(int $size, ColorInterface $backgroundColor): void
    {
        ...
        $this->xmlWriter->writeAttribute(
            "fill",
            $this->getColorString($backgroundColor)
        );
    ...
    public function drawPathWithGradient(
        ...
        $gradientId = $this->createGradientFill(
            $gradient,
            $x,
            $y,
            $width,
    ...
    private function startPathElement(Path $path): void
    {
        ...
        foreach ($path as $op) {
            switch (true) {
                ...
                default:
                    throw new RuntimeException(
                        "Unexpected draw operation: " . get_class($op)
                    );
            }
        }
    ...
    private function createGradientFill(
        Gradient $gradient,
        float $x,
        float $y,
        float $width,
        ...
        switch ($gradient->getType()) {
            case GradientType::HORIZONTAL():
                $this->xmlWriter->writeAttribute(
                    "x1",
                    (string) round($x, self::PRECISION)
                );
                $this->xmlWriter->writeAttribute(
                    "y1",
                    (string) round($y, self::PRECISION)
                );
                $this->xmlWriter->writeAttribute(
                    "x2",
                    (string) round($x + $width, self::PRECISION)
                );
                $this->xmlWriter->writeAttribute(
                    "y2",
                    (string) round($y, self::PRECISION)
            ...
            case GradientType::VERTICAL():
                ...
                $this->xmlWriter->writeAttribute(
                    "y2",
                    (string) round($y + $height, self::PRECISION)
                );
                break;
            ...
            case GradientType::RADIAL():
                $this->xmlWriter->writeAttribute(
                    "cx",
                    (string) round(($x + $width) / 2, self::PRECISION)
                );
                $this->xmlWriter->writeAttribute(
                    "cy",
                    (string) round(($y + $height) / 2, self::PRECISION)
                );
                $this->xmlWriter->writeAttribute(
                    "r",
                    (string) round(max($width, $height) / 2, self::PRECISION)
                );
                break;
        }
