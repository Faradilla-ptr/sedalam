...
class PusherCrypto
{
     ...
     */
    public static function parse_master_key(
        $encryption_master_key_base64
    ): string {
        if (!function_exists("sodium_crypto_secretbox")) {
            throw new PusherException(
                "To use end to end encryption, you must either be using PHP 7.2 or greater or have installed the libsodium-php extension for php < 7.2."
            );
        }
        ...
        if ($encryption_master_key_base64 !== "") {
            ...
            if ($decoded_key === false) {
                throw new PusherException(
                    "encryption_master_key_base64 must be a valid base64 string"
                );
            }
    ...
    public function decrypt_event(object $event): object
    {
        ...
        if (!$decrypted_payload) {
            throw new PusherException(
                "Decryption of the payload failed. Wrong key?"
            );
        }
    ...
    public function generate_shared_secret(string $channel): string
    {
        if (!self::is_encrypted_channel($channel)) {
            throw new PusherException(
                "You must specify a channel of the form private-encrypted-* for E2E encryption. Got " .
                    $channel
            );
        }
    ...
    public function encrypt_payload(string $channel, string $plaintext): string
    {
        if (!self::is_encrypted_channel($channel)) {
            throw new PusherException(
                "Cannot encrypt plaintext for a channel that is not of the form private-encrypted-*. Got " .
                    $channel
            );
        }
     ...
     * @throws \SodiumException
     */
    public function decrypt_payload(
        string $payload,
        string $nonce,
     ...
     */
    private function format_encrypted_message(
        string $nonce,
        string $ciphertext
    ): string {
    ...
    private function parse_encrypted_message(string $payload): object
    {
        ...
        $decoded_payload->ciphertext = base64_decode(
            $decoded_payload->ciphertext
        );
        if (
            $decoded_payload->ciphertext === "" ||
            strlen($decoded_payload->nonce) !==
                SODIUM_CRYPTO_SECRETBOX_NONCEBYTES
        ) {
            throw new PusherException(
                "Received a payload that cannot be parsed."
            );
        }
