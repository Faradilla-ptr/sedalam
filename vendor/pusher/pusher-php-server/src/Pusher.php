...
class Pusher implements LoggerAwareInterface, PusherInterface
{
    ...
    public function __construct(
        ...
        $this->settings["host"] = preg_replace(
            "/http[s]?\:\/\//",
            "",
            $this->settings["host"],
            1
    ...
    private function check_compatibility(): void
    {
        if (!extension_loaded("json")) {
            throw new PusherException(
                "The Pusher library requires the PHP JSON module. Please ensure it is installed"
            );
        }
     ...
     */
    public static function array_implode(
        string $glue,
        string $separator,
        $array
    ): string {
    ...
    public function make_request(
        ...
        if (!$data_encoded) {
            $this->log(
                "Failed to perform json_encode on the the provided data: {error}",
                [
    ...
    public function trigger(
        ...
        $post_value = $this->make_trigger_body(
            $channels,
            $event,
            $data,
            $params,
        ...
        $this->log("trigger POST: {post_value}", compact("post_value"));
        return $this->process_trigger_result(
            $this->post("/events", $post_value)
        );
    }
    ...
    public function triggerAsync(
        ...
        $this->log("trigger POST: {post_value}", compact("post_value"));
        return $this->postAsync("/events", $post_value)->then(function (
            $result
        ) {
     ...
     * @throws PusherException
     */
    public function sendToUserAsync(
        string $user_id,
        string $event,
        $data,
        bool $already_encoded = false
     ...
     * @deprecated in favour of of trigger and triggerAsync
     */
    public function make_batch_request(
        array $batch = [],
        bool $already_encoded = false
    ): Request {
        foreach ($batch as $key => $event) {
            ...
            if (PusherCrypto::is_encrypted_channel($event["channel"])) {
                $batch[$key]["data"] = $this->crypto->encrypt_payload(
                    $event["channel"],
                    $data
                );
            } else {
    ...
    public function triggerBatch(
        ...
        $this->log("trigger POST: {post_value}", compact("post_value"));
        return $this->process_trigger_result(
            $this->post("/batch_events", $post_value)
        );
    }
     ...
     * @throws PusherException
     */
    public function triggerBatchAsync(
        array $batch = [],
        bool $already_encoded = false
    ): PromiseInterface {
        $post_value = $this->make_trigger_batch_body($batch, $already_encoded);
        $this->log("trigger POST: {post_value}", compact("post_value"));
        return $this->postAsync("/batch_events", $post_value)->then(function (
            $result
        ) {
            return $this->process_trigger_result($result);
        });
     ...
     *
     */
    public function terminateUserConnectionsAsync(
        string $user_id
    ): PromiseInterface {
        $this->validate_user_id($user_id);
    ...
    public function authenticateUser(
        ...
        $signature = hash_hmac(
            "sha256",
            "$socket_id::user::$serialized_user_data",
            $this->settings["secret"],
            false
        );
    ...
    public function authorizeChannel(
        ...
        if (PusherCrypto::is_encrypted_channel($channel)) {
            if (!is_null($this->crypto)) {
                $signature["shared_secret"] = base64_encode(
                    $this->crypto->generate_shared_secret($channel)
                );
            } else {
                throw new PusherException(
                    "You must specify an encryption master key to authorize an encrypted channel"
                );
            }
        }
     ...
     * @throws PusherException Throws exception if $channel is invalid or above or $socket_id is invalid
     */
    public function authorizePresenceChannel(
        string $channel,
        string $socket_id,
        string $user_id,
        $user_info = null
        ...
        try {
            return $this->authorizeChannel(
                $channel,
                $socket_id,
                json_encode($user_data, JSON_THROW_ON_ERROR)
            );
        } catch (\JsonException $e) {
            throw new PusherException("Data encoding error.");
        }
    ...
    public function presenceAuth(
        ...
        return $this->authorizePresenceChannel(
            $channel,
            $socket_id,
            $user_id,
            $user_info
    ...
    public function webhook(array $headers, string $body): object
    {
        ...
        try {
            ...
            $this->log(
                "Unable to decrypt webhook event payload.",
                null,
                LogLevel::WARNING
            );
            throw new PusherException("Data encoding error.");
            ...
            if (PusherCrypto::is_encrypted_channel($event->channel)) {
                if (!is_null($this->crypto)) {
                    ...
                    if ($decryptedEvent === false) {
                        $this->log(
                            "Unable to decrypt webhook event payload. Wrong key? Ignoring.",
                            null,
                            LogLevel::WARNING
                        );
                        continue;
                    ...
                    $this->log(
                        "Got an encrypted webhook event payload, but no master key specified. Ignoring.",
                        null,
                        LogLevel::WARNING
                    );
                }
    ...
    public function verifySignature(array $headers, string $body): void
    {
        ...
        throw new PusherException(
            sprintf(
                "Received WebHook with invalid signature: got %s.",
                $x_pusher_signature
            )
        );
    }
    ...
    private function make_event(
        ...
        if ($has_encrypted_channel) {
            if (PusherCrypto::has_mixed_channels($channels)) {
                throw new PusherException(
                    "You cannot trigger to encrypted and non-encrypted channels at the same time"
                );
            } else {
                try {
                    $data_encoded = $this->crypto->encrypt_payload(
     ...
     * @return string
     */
    private function make_trigger_body(
        $channels,
        string $event,
        $data,
        array $params = [],
     ...
     * @return string
     */
    private function make_trigger_batch_body(
        array $batch = [],
        bool $already_encoded = false
    ): string {
        foreach ($batch as $key => $event) {
    ...
    {
        if (
            property_exists($result, "channels") &&
